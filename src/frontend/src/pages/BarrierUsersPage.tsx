...
const BarrierUsersPage: React.FC = () => {
    ...
    const fetchUsers = async (token = accessToken) => {
        ...
        try {
            const res = await fetch(`/api/admin/barriers/${barrier_id}/users/?${params.toString()}`, {
                headers: {
    ...
    const handleDeleteUser = async (userId: number) => {
        ...
        try {
            const res = await fetch(`/api/admin/barriers/${barrier_id}/users/${userId}/`, {
                method: "DELETE",
                headers: { Authorization: `Bearer ${accessToken}` },
            });
            if (res.ok) {
                ...
                const data = await res.json();
                setDeleteErrors((prev) => ({ ...prev, [userId]: data.detail || "Ошибка удаления пользователя." }));
            }
        } catch {
            setDeleteErrors((prev) => ({ ...prev, [userId]: "Ошибка сети при удалении." }));
        }
    };

    const handleBlockUser = async (userId: number) => {
        if (!blockReason.trim()) {
            setBlockErrors((prev) => ({ ...prev, [userId]: "Укажите причину блокировки." }));
            return;
        }
        try {
            ...
            if (res.ok) {
                ...
                const data = await res.json();
                setBlockErrors((prev) => ({ ...prev, [userId]: data.detail || "Ошибка блокировки." }));
            }
        } catch {
            setBlockErrors((prev) => ({ ...prev, [userId]: "Ошибка сети при блокировке." }));
        }
    };

    useEffect(() => { fetchBarrierInfo(); }, []);
    useEffect(() => { fetchUsers(); }, [ordering, page, search]);

    return (
        <div style={styles.container}>
            <button
                style={styles.backButton}
                onClick={() => navigate("/admin-barrier-page", {
                ...
                <ul style={styles.list}>
                    {users.map((user) => (
                        <li key={user.id} style={styles.userCard}>
                            ...
                            <button
                                style={{ ...styles.button, backgroundColor: "#ffe3e3", color: "#d9534f" }}
                                onClick={() => handleDeleteUser(user.id)}
                            >Удалить из шлагбаума</button>
