...
const ChangePhonePage: React.FC = () => {
    ...
    const translateError = (error: string): string => {
        const map: { [key: string]: string } = {
            "Verification code was already sent. Try again later.": "Код уже был отправлен. Попробуйте позже.",
    ...
    const handleSendOldCode = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                const retryMsg = data.retry_after ? ` Повторите через ${data.retry_after} секунд.` : "";
                setError(translateError(data.detail || "Ошибка отправки кода") + retryMsg);
            }
        } catch (e) {
    ...
    const handleSendNewCode = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                const retryMsg = data.retry_after ? ` Повторите через ${data.retry_after} секунд.` : "";
                setError(translateError(data.detail || "Ошибка отправки кода на новый номер") + retryMsg);
            }
        } catch (e) {
            setError("Ошибка сети");
        }
    ...
    const handleVerifyNewCodeAndSubmit = async () => {
        try {
            ...
            if (finalRes.ok) {
                ...
                setError(translateError(result.detail || "Не удалось изменить номер телефона"));
            }
        } catch (e) {
            setError("Ошибка сети");
        }
    ...
    return (
        <div style={styles.container}>
            <button style={styles.backIcon} onClick={() => navigate("/user")}> <FaArrowLeft /> Назад </button>
            <div style={styles.card}>
                ...
                {step === 1 && (
                    <>
                        ...
                        <button style={styles.button} onClick={handleSendOldCode}>Получить код</button>
                    </>
                )}
                ...
                {step === 2 && (
                    <>
                        ...
                        <input style={styles.input} value={oldCode} onChange={(e) => setOldCode(e.target.value)} />
                        <button style={styles.button} onClick={handleVerifyOldCode}>Подтвердить</button>
                        <button style={{ ...styles.button, backgroundColor: "#d7c4ed", color: "#5a4478" }} onClick={handleResendOldCode}>Получить код повторно</button>
                    </>
                )}
                ...
                {step === 3 && (
                    <>
                        ...
                        />
                        <button style={styles.button} onClick={handleSendNewCode}>Получить код</button>
                    </>
                )}
                ...
                {step === 4 && (
                    <>
                        ...
                        <input style={styles.input} value={newCode} onChange={(e) => setNewCode(e.target.value)} />
                        <button style={styles.button} onClick={handleVerifyNewCodeAndSubmit}>Подтвердить и изменить номер</button>
                        <button style={{ ...styles.button, backgroundColor: "#d7c4ed", color: "#5a4478" }} onClick={handleResendNewCode}>Получить код повторно</button>
                    </>
                )}
