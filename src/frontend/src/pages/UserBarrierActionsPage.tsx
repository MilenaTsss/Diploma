...
const UserBarrierActionsPage: React.FC = () => {
    ...
    const fetchActions = async () => {
        ...
        try {
            const res = await fetch(`/api/barriers/${barrierId}/actions/?${params.toString()}`, {
    ...
    const fetchActionDetail = async (actionId: number) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (data.phone) {
                    const phoneRes = await fetch(`/api/phones/${data.phone}/`, {
                        headers: { Authorization: `Bearer ${accessToken}` },
                    });
            ...
            console.error("[Action Detail Error]", e);
    ...
    return (
        <div style={styles.container}>
            ...
            <h2 style={styles.title}>История изменений</h2>

            <div style={styles.filters}>
                <select
                    value={filters.action_type || ""}
                    onChange={(e) => setFilters((f: any) => ({ ...f, action_type: e.target.value }))}
                    style={styles.input}
                >
                ...
                <input
                    ...
                    onChange={(e) => setFilters((f: any) => ({ ...f, created_from: e.target.value }))}
                    style={styles.input}
                />
                <input
                    type="datetime-local"
                    value={filters.created_to || ""}
                    onChange={(e) => setFilters((f: any) => ({ ...f, created_to: e.target.value }))}
                    style={styles.input}
                />
            </div>
            ...
            {actions.map((a) => (
                ...
                >
                    <strong>#{a.id}</strong> — {actionTypeLabels[a.action_type] || a.action_type} <br />
            ...
            {selectedAction && (
                <div style={styles.detailBox}>
                    ...
                    <p><strong>Тип:</strong> {actionTypeLabels[selectedAction.action_type] || selectedAction.action_type}</p>
                    <p><strong>Автор:</strong> {selectedAction.author}</p>
                    <p><strong>Создано:</strong> {selectedAction.created_at}</p>
                    <p><strong>Причина:</strong> {reasonLabels[selectedAction.reason] || selectedAction.reason || "-"}</p>
                    {phoneDetails && (
                        <>
                            <p><strong>Телефон:</strong> {phoneDetails.phone}</p>
                            <p><strong>Тип:</strong> {phoneDetails.type}</p>
                        </>
                    )}
                    <button style={styles.closeBtn} onClick={() => { setSelectedAction(null); setPhoneDetails(null); }}>Скрыть детали</button>
                </div>
            )}
...
export default UserBarrierActionsPage;
